...
exports.dashboard = async (req, res) => {
  try {
    ...
    // 5. TOP PRODUCTS (từ dữ liệu thực trong orders)
    try {
      // Sử dụng aggregation để lấy top products từ orders thực tế
      ...
      // Tính phần trăm dựa trên tổng doanh thu
      const totalTopProductsRevenue = topProductsData.reduce((sum, product) => sum + product.totalRevenue, 0)

      topProducts = topProductsData.map((product) => ({
        name: product.productName || "Unknown Product",
        totalSold: product.totalQuantity,
        percentage:
          totalTopProductsRevenue > 0 ? Math.round((product.totalRevenue / totalTopProductsRevenue) * 100) : 0,
        revenue: product.totalRevenue,
      }))

      console.log("✅ Top Products from real orders:", topProducts)
    } catch (error) {
      console.log("Error getting top products:", error.message)

      // Fallback nếu không có orders
      const products = await Product.find().populate("category").sort({ createdAt: -1 }).limit(5)
    ...
    // 6. SALES BY CATEGORY (từ dữ liệu thực trong orders)
    try {
      // Sử dụng aggregation để lấy doanh thu theo category từ orders thực tế
      ...
      const totalCategoryRevenue = salesByCategoryData.reduce((sum, cat) => sum + cat.totalRevenue, 0)

      salesByCategory = salesByCategoryData.map((category, index) => ({
        name: category.categoryName,
        percentage: totalCategoryRevenue > 0 ? Math.round((category.totalRevenue / totalCategoryRevenue) * 100) : 0,
        color: colors[index] || "#6c757d",
        revenue: category.totalRevenue,
      }))

      console.log("✅ Sales by Category from real orders:", salesByCategory)
    } catch (error) {
    ...
    // 7. RECENT ACTIVITIES
    try {
      recentActivities = await Product.find().populate("category").sort({ updatedAt: -1 }).limit(5)
    } catch (error) {
...
exports.updateUser = async (req, res) => {
  try {
    ...
    const userId = req.params.id
    const { fullName, email, password, role, phone, address, status } = req.body
    ...
    const mappedStatus = req.body.status === "inactive" ? "blocked" : req.body.status
...
exports.addProduct = async (req, res) => {
  try {
    ...
    const parsedDiscountPercentage = discountPercentage ? Number.parseFloat(discountPercentage) : 0 // default 0;
    ...
    const commonSpecs = {
      ...
      warrantyInfo: {
        durationInMonths: Number.parseInt(req.body["commonSpecs[warrantyInfo][durationInMonths]"]) || 0,
    ...
    const discountPrice =
      parsedDiscountPercentage > 0 ? parsedPrice - parsedPrice * (parsedDiscountPercentage / 100) : parsedPrice
...
exports.priceUpdate = async (req, res) => {
  try {
    ...
    if (minQuantity && !isNaN(minQuantity)) filter.quantity = { $gte: Number.parseInt(minQuantity) }

    // --- Mode 1: Update Base Price ---
    if (mode === "basePriceUpdate") {
      const { actionType, adjustValue } = req.body

      const value = Number.parseFloat(adjustValue)
    ...
    // --- Mode 2: Set Discount Percentage ---
      ...
      const discountValue = Number.parseFloat(discountPercentage)
