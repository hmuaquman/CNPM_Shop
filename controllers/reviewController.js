...
const addReview = asyncHandler(async (req, res) => {
  ...
  const username =
    req.user && req.user.userName ? req.user.userName : req.user.username || "";
  ...
  try {
    ...
    if (error.code === 11000) {
      req.flash(
        "error_msg",
        "Bạn đã đánh giá sản phẩm này rồi. Nếu bạn gặp lỗi này do dữ liệu cũ, vui lòng liên hệ admin để xóa review cũ."
      );
